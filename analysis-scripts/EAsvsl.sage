...
f=open("/home/mazur/worldline/EAsvsLambda.dat", 'r')
...
plt.clf()
plt.errorbar(l,[-0.5*EAferm[i] + EAclass[i] for i in range(len(EAferm))],eEAferm,marker='.', color='k', ms=8, linewidth=0.0, elinewidth=1)
...
ax.set_ylim(-600,0)
ax.set_xlim(0.1,0.6)
plt.savefig('EAsvsl.eps')
plt.savefig('EAsvsl.png')
